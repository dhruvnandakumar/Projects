...
public class AStarDiv_914032009 implements AIModule {
    ...
    private double penalty(Point start, Point end, Point current, TerrainMap map) {
    ...
    public List<Point> createPath(final TerrainMap map) {
        ...
        HashMap<Point, Double> closed = new HashMap<>();
        ...
        while (!fringe.isEmpty()) {
            ...
            for (Point neighbor : neighbors) {
                ...
                if (!closed.containsKey(neighbor) || closed.get(neighbor) > tentativeCost) {
                    pathTile successor = new pathTile(map, neighbor, tentativeCost);
                    successor.from = current;
                    closed.put(neighbor, tentativeCost); //Safe; java Hashmap does not allow duplicate keys
    ...
    public class pathTile {
        ...
        public pathTile(final TerrainMap map, final Point p, double gValue) {
            ...
            this.fVal = (this.gVal + this.hVal) /** penalty(map.getStartPoint(), map.getEndPoint(), this.point,map)*/;
            this.from = null;
